...
(function(global) {
    ...
    function generateDynamicTable(){
        var noOfItems = scoreList.length;
        var title = document.createElement("h4");
        title.innerHTML = "<span>Total # of files: <strong>" + noOfItems + "</strong></span>";
        var divContainerTop = document.getElementById("fileCount");
		divContainerTop.innerHTML = "";
		divContainerTop.appendChild(title);
		...
		if (noOfItems > 0){
			...
			for (var i = 0; i < col.length; i++) {
                    ...
                    switch(i) {
                        case 0:
                            ...
                            break;
                        case 1:
                            th.innerHTML = "# of Faces Detected";
                            th.setAttribute('style', 'text-align: center;');
                            break;
                        case 2:
                            th.innerHTML = "Average Happiness Score";
                            th.setAttribute('style', 'text-align: center;');
                    } 
					hRow.appendChild(th);
			}
			...
			for (var i = 0; i < noOfItems; i++) {
                    ...
                    if (i < 10){
                        bRow.setAttribute('class', 'active');
                    }
					for (var j = 0; j < col.length; j++) {
                        var td = document.createElement("td");
                        if (j == 2){
                            td.innerHTML = scoreList[i][col[j]].toFixed(5);
                        } else {
                            td.innerHTML = scoreList[i][col[j]];
                        }
                        if (j > 0){
			...
			var divContainer = document.getElementById("rankedList");
        ...
        function handleSuccessResponse(id, xhrOrXdr, isError) {
            ...
            try {
                ...
                if (isError || parsedResponse && (parsedResponse.error || parsedResponse.success === false)) {
                    ...
                    var scoreJson = JSON.parse(parsedResponse);
                    if ( scoreJson.numofFace == 0 ){
                        scoreJson.avgScore = -1;
                    }
                    scoreList.push(scoreJson);
                    console.log(scoreList);
                    scoreList.sort(scoreCompare);
                    console.log(scoreList);
                    //generateDynamicTable();
